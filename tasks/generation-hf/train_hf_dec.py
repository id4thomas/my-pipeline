...
# transformers
from transformers import AutoModel, AutoTokenizer, AutoModelForCausalLM, AutoModelWithLMHead
...
def run(args, prefix_list):
    ...
    tr_ds = tr_ds.map(
        ...
        lambda batch: batch_tokenize_preprocess_dec_cont(
            batch, tokenizer, config.max_length
        ),
        batched=True,
        # remove_columns=[""],
    )
